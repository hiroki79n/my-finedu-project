...
- 🎯 **"Earn to Invest" UX (NEW!)**: 稼ぐ→投資するの循環を直感的に理解できるUI
...
### ✅ ホーム画面（"Earn to Invest" Cycle UX - NEW! 🎯）
- **ダイナミック・ヒーローセクション（キャッシュフロー循環の視覚化）**:
  - 左: ⚔️ 冒険で稼ぐ（Work） - 所持金が少ない時に強調
  - 中央: 💰 お財布（Cash） - 現金残高をリアルタイム表示
  - 右: 🏛️ 市場で投資（Invest） - 所持金が十分な時に強調
  - コイン飛翔アニメーション: 稼ぐ→お財布→投資の流れを演出
- **スマート・アクションボタン（Context-Aware FAB）**:
  ...
  - 所持金 >= ¥1,000の場合:
    ...
    - アイコン: 📈
    - テキスト: 「マーケットで投資する！」
    - アニメーション: Sparkle（キラキラ）
    - 遷移先: マーケット画面
- **クエスト＆ニュース・カード**:
  - 今日のニュース: 市場トレンドの通知
  - デイリーミッション: ボーナス獲得のヒント
- **総資産表示**: 現金残高と株式評価額の合計をリアルタイム表示
  - 総資産 = 現金残高 + 株式評価額（保有数量 × 現在価格の合計）
  - 株価変動に応じて自動的に再計算
- **コンパクトヘッダー**: レベル、ストリーク表示
- **サブアクション**: 資産、履歴、設定への素早いアクセス
...
### ✅ 冒険マップ画面（NEW! 🗺️）
- **クエストシステム**: 報酬を獲得できる冒険クエスト
- **難易度別クエスト**:
  - 初級（📚 株式投資の基礎）: ¥500 + 50 XP
  - 中級（🗺️ マーケット探検）: ¥1,000 + 100 XP
  - 上級（👹 ボスチャレンジ）: ¥2,000 + 200 XP
- **クエスト詳細モーダル**: 報酬と経験値を確認してから挑戦
- **クイズ画面へのシームレスな遷移**: クエスト開始でクイズへ
