...
### ✅ ホーム画面
- **総資産表示（NEW! 💰）**: 現金残高と株式評価額の合計をリアルタイム表示
  - 総資産 = 現金残高 + 株式評価額（保有数量 × 現在価格の合計）
  - 現金残高と株式評価額の内訳をアニメーション表示
  - 株価変動に応じて自動的に再計算
- XPプログレスバー（次のレベルまでの進捗表示）
- ストリーク表示（連続ログイン日数）
- 資産残高のアニメーション表示（カウントアップ効果）
- 5つのメインアクション：マーケット、クイズ、ポートフォリオ、取引履歴、設定
...
- **マーケット更新設定**:
  ...
  - スライダーで直感的に設定
  - 超高速（10-20秒）、高速（20-40秒）、標準（40-80秒）、低速（80-120秒）
- **資産評価設定**:
  - 総資産の計算ロジックの説明
  - リアルタイム評価の仕組み
...
- **テーブル構成**:
  ...
  - `user_settings` (NEW!): ユーザー設定（市場更新間隔、自動更新フラグ）
